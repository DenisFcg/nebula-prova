...
class DeclaracaoFuncao(ASTNode):
    ...
    corpo: List[ASTNode]
    def __repr__(self):
        return f"DeclaracaoFuncao({self.nome}, params={len(self.parametros)}, corpo={len(self.corpo)} stmts)"


@dataclass
class Chamada(ASTNode):
    callee: ASTNode
    argumentos: List[ASTNode]
    def __repr__(self):
        return f"Chamada({self.callee}, args={len(self.argumentos)})"


@dataclass
class Retornar(ASTNode):
    valor: Optional[ASTNode] = None
    def __repr__(self):
