...
class FuncaoNebula:
    def __init__(self, nome: str, parametros: List[str], corpo: List[ASTNode]):
        self.nome = nome
...
class Interpretador:
    ...
    def _executar_DeclaracaoFuncao(self, node: DeclaracaoFuncao):
        funcao = FuncaoNebula(node.nome, node.parametros, node.corpo)
        self.escopos[-1][node.nome] = funcao
        return funcao
    ...
    def _executar_Chamada(self, node: Chamada):
        callee = self.executar(node.callee)

        if not isinstance(callee, FuncaoNebula):
            raise ErroExecucao("Tentativa de chamar um valor que não é função")

        if len(node.argumentos) != len(callee.parametros):
            raise ErroExecucao(
                f"Função '{callee.nome}' esperava {len(callee.parametros)} argumento(s), recebeu {len(node.argumentos)}"
            )

        argumentos = [self.executar(arg) for arg in node.argumentos]
        escopo_local = dict(zip(callee.parametros, argumentos))
        self.escopos.append(escopo_local)

        try:
            for stmt in callee.corpo:
                self.executar(stmt)
        except RetornoFuncao as retorno:
    ...
    def _executar_Retornar(self, node: Retornar):
        valor = None
    ...
    def _executar_BinOp(self, node: BinOp):
        ...
        if op == TipoToken.SOMA:
            if isinstance(esq, str) and isinstance(dir_val, str):
                return esq + dir_val
            if isinstance(esq, str) or isinstance(dir_val, str):
                raise ErroExecucao("Operação '+' exige operandos compatíveis")
            return esq + dir_val
