...
class Parser:
    ...
    def parse(self) -> Programa:
        ...
        while self._atual().tipo != TipoToken.EOF:
            try:
                ...
                print(f"[!] {e}")
                self._sincronizar([TipoToken.PONTO_VIRGULA, TipoToken.FECHA_CHAVE, TipoToken.SENAO])
    ...
    def _parse_statement(self) -> Optional[ASTNode]:
        ...
        if token.tipo in (TipoToken.SENAO, TipoToken.FECHA_CHAVE, TipoToken.EOF):
            return None
        ...
        elif token.tipo == TipoToken.DEFINIR:
            return self._parse_declaracao_funcao()
        elif token.tipo == TipoToken.RETORNAR:
        ...
        elif token.tipo == TipoToken.ABRE_CHAVE:
    ...
    def _parse_declaracao_funcao(self) -> DeclaracaoFuncao:
        self._esperar(TipoToken.DEFINIR)
        nome = self._esperar(TipoToken.IDENTIFICADOR).valor
        self._esperar(TipoToken.ABRE_PARENTESES)

        parametros = []
        if self._atual().tipo != TipoToken.FECHA_PARENTESES:
            parametros.append(self._esperar(TipoToken.IDENTIFICADOR).valor)
            while self._atual().tipo == TipoToken.VIRGULA:
                self._avancar()
                parametros.append(self._esperar(TipoToken.IDENTIFICADOR).valor)

        self._esperar(TipoToken.FECHA_PARENTESES)
        corpo = self._parse_bloco()
        return DeclaracaoFuncao(nome, parametros, corpo.statements)

    def _parse_retornar(self) -> Retornar:
        self._esperar(TipoToken.RETORNAR)
        valor = None
        if self._atual().tipo != TipoToken.PONTO_VIRGULA:
            valor = self._parse_expressao()
        self._esperar(TipoToken.PONTO_VIRGULA)
    ...
    def _parse_atribuicao_ou_chamada(self) -> ASTNode:
        ...
        elif self._atual().tipo == TipoToken.ABRE_PARENTESES:
            chamada = self._parse_chamada(referencia)
            self._esperar(TipoToken.PONTO_VIRGULA)
            return chamada
        else:
            return referencia

    def _parse_chamada(self, callee: ASTNode) -> Chamada:
        self._esperar(TipoToken.ABRE_PARENTESES)
        argumentos = []
        ...
        self._esperar(TipoToken.FECHA_PARENTESES)
        return Chamada(callee, argumentos)

    def _parse_posfixo(self, node: ASTNode) -> ASTNode:
        while self._atual().tipo == TipoToken.ABRE_PARENTESES:
            node = self._parse_chamada(node)
        return node
    ...
    def _parse_bloco(self) -> Programa:
        self._esperar(TipoToken.ABRE_CHAVE)
        statements = []

        while self._atual().tipo not in (TipoToken.FECHA_CHAVE, TipoToken.EOF):
            stmt = self._parse_statement()
            if stmt:
                statements.append(stmt)

        self._esperar(TipoToken.FECHA_CHAVE)
        return Programa(statements)
    ...
    def _parse_primario(self) -> ASTNode:
        ...
        if token.tipo == TipoToken.IDENTIFICADOR:
            self._avancar()
            return self._parse_posfixo(Variavel(token.valor))

        if token.tipo == TipoToken.ABRE_PARENTESES:
            self._avancar()
            node = self._parse_expressao()
            self._esperar(TipoToken.FECHA_PARENTESES)
            return self._parse_posfixo(node)
